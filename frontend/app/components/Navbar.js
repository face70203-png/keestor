...
import { Key, LayoutDashboard, LogIn, LogOut, ShieldCheck, ShoppingCart, Globe } from "lucide-react";
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-gray-200 py-4 px-6 md:px-12 flex items-center justify-between">
      ...
      <div className="flex items-center gap-6">
        <div className="hidden md:flex items-center gap-6">
            <Link href="/products" className="text-slate-600 hover:text-primary font-bold transition-colors">{t.products}</Link>
            <div className="relative group cursor-pointer py-2">
                <span className="text-slate-600 group-hover:text-primary font-bold transition-colors flex items-center gap-1">
                    {lang === 'ar' ? 'الأقسام' : 'Categories'} <svg className="w-4 h-4 mt-0.5 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                </span>
            ...
            <Link href="/about" className="text-slate-600 hover:text-primary font-bold transition-colors">{t.about}</Link>
        </div>
        
        <div className="flex items-center gap-4">
            <button onClick={toggleLanguage} className="p-2 rounded-full hover:bg-slate-100 transition-colors text-slate-700 flex items-center gap-2 text-xs font-black uppercase">
                <Globe size={18} className="text-primary"/> {lang === 'en' ? 'Arabic' : 'English'}
            </button>
